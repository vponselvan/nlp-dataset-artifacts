...
NEGATION_WORDS = {
    ...
    "couldn't", "shouldn't", "mustn't", "haven't", "hasn't", "hadn't", "aren't", "isn't", "wasn't", "weren't",
...
def create_negated_context(context: str, max_attempts: int = 3) -> str:
    ...
    templates = random.sample(NEGATION_TEMPLATES, min(max_attempts, len(NEGATION_TEMPLATES)))
...
def generate_contrastive_pairs(
    ...
    for i, ex in enumerate(examples):
        ...
        if not has_neg and random.random() < augmentation_ratio:
            ...
            additive_ex['context'] = create_additive_negation(ex['context'], answer_text)
    ...
    print(f"  - Already containing negation: {stats['already_negated']} ({stats['already_negated']/stats['total']*100:.1f}%)")
    print(f"  - Positive (affirmative) examples: {stats['positive_examples']} ({stats['positive_examples']/stats['total']*100:.1f}%)")
    ...
    print(f"Negation examples (weighted {negation_weight}x): {stats['already_negated'] + stats['additive_negation'] + stats['transformative_negation']}")
    print("=" * 70)
