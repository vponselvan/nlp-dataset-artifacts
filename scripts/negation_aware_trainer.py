...
class NegationAwareTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if isinstance(outputs, dict):
            ...
            start_logits = outputs.start_logits if hasattr(outputs, 'start_logits') else None
        ...
        if loss_weights is not None:
            ...
            if isinstance(loss_weights, torch.Tensor):
                ...
                if self.log_weight_stats and self.weight_stats['total_steps'] % 100 == 0:
                    logger.info(
    ...
    def _compute_weighted_qa_loss(
        ...
        if start_positions is None or end_positions is None:
            raise ValueError("start_positions and end_positions must be in inputs for QA loss")
...
class NegationAwareQATrainer(NegationAwareTrainer):
    ...
    def evaluate(self, eval_dataset=None, eval_examples=None, ignore_keys=None, metric_key_prefix="eval"):
        ...
        if self.post_process_function is not None and eval_examples is not None:
            output = self.post_process_function(eval_examples, eval_dataset, output.predictions)
