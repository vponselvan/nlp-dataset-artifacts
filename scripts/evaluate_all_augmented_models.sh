#!/bin/bash
# Evaluate all 5 augmented models on both AddSent and SQuAD
# This will take approximately 1 hour

set -e

echo "=========================================="
echo "Evaluating All 5 Augmented Models"
echo "=========================================="
echo ""

# Configuration
ADDSENT_EVAL="./data/addsent_eval.jsonl"
SQUAD_EVAL="./data/squad.jsonl"
BATCH_SIZE=32
...
if [ ! -f "$SQUAD_EVAL" ]; then
    echo "❌ Error: SQuAD file not found: $SQUAD_EVAL"
    exit 1
fi
...
for ratio in "${RATIOS[@]}"; do
    model_path="./trained_model_adversarial_${ratio}_augmented"
...
evaluate_model() {
    local ratio=$1
    local model_path="./trained_model_adversarial_${ratio}_augmented"
    local eval_dir="./evaluation/adversarial_${ratio}_augmented"
    
    echo "=========================================="
    echo "Evaluating ${ratio} Augmented Model"
    echo "=========================================="
    echo ""
...
ls -d ./evaluation/adversarial_*_augmented 2>/dev/null | awk '{print "  " NR ". " $1}'
...
echo "  python3 scripts/compare_augmented_models.py"
echo ""
echo "View individual results:"
for ratio in "${RATIOS[@]}"; do
    eval_file="./evaluation/adversarial_${ratio}_augmented/eval_metrics.json"
    if [ -f "$eval_file" ]; then
        echo "  cat $eval_file"
    fi
done
